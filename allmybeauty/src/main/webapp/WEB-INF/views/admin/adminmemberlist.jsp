          ...
          <div class="container">
                 <div class="row">
                     <div class="col-sm-10 col-sm-offset-2">
                     	   <div class="form-group">
                             <h4 class="font-alt mb-0">회원정보조회</h4>
                         </div>
            ...
            <div class="container" style="text-align:center">
             <div class="row">
              ...
              <!-- 회원 검색 -->
           <form class="form-inline" id="frmSendValue" action="${contextPath }/admin/adminmemberlist" method="get" name="frmSendValue">
             
                 <div class="form-group" >
                      <label class="sr-only">frmSendValues</label>
                       <select class="form-control" id="selectAmount" name="rowAmountPerPage">
                           <option value="10" <c:out value="${adminpagingCreator.adminMemberPagingDTO.rowAmountPerPage eq '10' 
                                                                          ? 'selected' : ''}"/>>10개</option>
                           <option value="20" <c:out value="${adminpagingCreator.adminMemberPagingDTO.rowAmountPerPage eq '20' 
                                                                          ? 'selected' : ''}"/>>20개</option>
                            <option value="50" <c:out value="${adminpagingCreator.adminMemberPagingDTO.rowAmountPerPage eq '50' 
                                                                          ? 'selected' : ''}"/>>50개</option>
                            <option value="100" <c:out value="${adminpagingCreator.adminMemberPagingDTO.rowAmountPerPage eq '100' 
                                                                          ? 'selected' : ''}"/>>100개</option>
                                                                          
                       </select>
                       
                       <select class="form-control" id="selectScope" name="scope">
                          <option value="" <c:out value="${adminpagingCreator.adminMemberPagingDTO.scope == null 
                                                                          ? 'selected' : ''}"/>>검색범위</option>
                            <option value="I" <c:out value="${adminpagingCreator.adminMemberPagingDTO.scope eq 'I' 
                                                                          ? 'selected' : ''}"/>>아이디</option>
                             <option value="A" <c:out value="${adminpagingCreator.adminMemberPagingDTO.scope eq 'A' 
                                                                          ? 'selected' : ''}"/>>주소</option>
                             <option value="N" <c:out value="${adminpagingCreator.adminMemberPagingDTO.scope eq 'N' 
                                                                          ? 'selected' : ''}"/>>이름</option>
                       </select>
                       
                       <div class="input-group">
							<input class="form-control" id="inputKeyword" name="keyword" type="text" style="width:250px; height:35px;" placeholder="검색어를 입력하세요"
										value='<c:out value="${adminpagingCreator.adminMemberPagingDTO.keyword}" />' />&nbsp;
							<span class="input-group-btn"><!-- 전송버튼 -->
									<button class="btn btn-d btn-round" type="button" id="btnSearchGo">
                 ...
                 <input type='hidden' name='mid' class="AdminMember_mid" value="${adminpagingCreator.adminMemberPagingDTO.mid }">
           ...
           <div class="col-sm-2" style="float:left;">
            ...
            <span style="font-size:15px;"><strong>[ 전체 회원 수 &nbsp; <c:out value="${adminpagingCreator.rowAmountTotal }"/>명]</strong></span>
              <div class="col-sm-10" style="text-align: center; float:right;">
               ...
               <tbody>                   
                    ...
                    <%-- <tr class="MemberMid" data-mid ='<c:out value="${adminmember.mid}"/>'>	   --%>
                      <tr>
		         	         <td>
		         	              <span>${adminmember.mid }</span>
		         	         </td>
		         	         <td>
		         	             <span>${adminmember.mname }</span>
		         	         </td>           	         		                               
		         	         <td>
		         	              <span>${adminmember.mphone }</span>
		         	         </td>       
		         	         <td>
		         	              <span>${adminmember.maddress  } ${adminmember.mdetailAddress} </span>
		         	         </td>
		         	         <td>
		         	               <span><fmt:formatDate pattern="yyyy/MM/dd" value="${adminmember.mregDate}"/></span>
		         	         </td>
		         	         <td>
		         	               <button type="button" class="btn btn-default btn-circle btn-xs modify_btn" data-mid="${adminmember.mid}">수정</button>
		         	         </td>
			          </tr>
